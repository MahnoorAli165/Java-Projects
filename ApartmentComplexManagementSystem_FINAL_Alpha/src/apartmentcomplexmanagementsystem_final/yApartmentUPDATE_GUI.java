...
public class yApartmentUPDATE_GUI extends JFrame implements ActionListener
{
    public yApartmentUPDATE_GUI()
    {
        setVisible(true);
        setSize(700, 300);
        setResizable(false);
    ...
    JLabel heading = new JLabel("Add Record with Existing ID", JLabel.CENTER);
    ...
    JLabel RENTTFlabel = new JLabel("Enter Yearly Rent:");
        ...
        switch(e.getActionCommand())
        {
            case "UPDATE":
                try
                {
                    ...
                    r = new yApart(apartNo, floor, size, monthlyRent);
                    r.updateRecord();
                }
                catch(Exception ex)
                {
